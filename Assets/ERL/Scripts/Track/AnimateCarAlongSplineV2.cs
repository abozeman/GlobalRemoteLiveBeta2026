...
{
    public class AnimateCarAlongSplineV2 : MonoBehaviour
    {
        [SerializeField]
        ...
        [SerializeField]
        public GameObject m_CarToAnimate;
        ...
        public void Initialize()
        {
            ...
            if (m_SplineContainer == null && !TryGetComponent<SplineContainer>(out m_SplineContainer))
                if (m_CarToAnimate == null)
                    TryGetComponent<GameObject>(out m_CarToAnimate);
        }
        ...
        void Update()
        {
            try
            {
                ...
                m_CarToAnimate.transform.position.Set(
                    m_CarToAnimate.transform.position.x,
                    0f,
                    m_CarToAnimate.transform.position.z);
                if(m_arrow != null) m_arrow.transform.position = m_CarToAnimate.transform.position + new Vector3(0,.2f,0);
                ...
                if (m_arrow != null) m_arrow.transform.rotation = Quaternion.LookRotation(direction, rot * up);
            }
            catch (Exception)
            {

                Console.WriteLine("animateCar Exception");
            }
        }
    }
}
