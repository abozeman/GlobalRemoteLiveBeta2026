...
namespace Meta.XR.MRUtilityKitSamples.EnvironmentPanelPlacement
{
    ...
    public class EnvironmentPanelPlacement : MonoBehaviour
    {
        ...
        [SerializeField] private float _panelAspectRatio = 0.823f;   //1.16348773842
        ...
        private async void CreateSpatialAnchorAndSave(Transform target)
        {
            ...
            _panel.SetParent(target, false);
            //_trackPlatform.SetLocalPositionAndRotation(default, Quaternion.identity);
            _panel.SetLocalPositionAndRotation(default, _panel.rotation);
            ...
            if (_isGrabbing)
            {
                ...
                if (OVRInput.GetUp(_grabButton))
                {
                    ...
                    if (!_isRestoringAnchorTracking)
                    {
                        ...
                        if (_panel.localPosition.magnitude > 3f || _spatialAnchor == null || !_spatialAnchor.Localized)
                        {
                            if (_spatialAnchor != null)
                            {
